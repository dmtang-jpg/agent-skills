...
## 📁 目录规范（所有 agent 必须遵守，2026-08-01 建立）

根目录下每个 agent 有**专属目录**，共用文件**多放一份**到公共目录：

| 目录 | 用途 |
|------|------|
| `/树上的AI/` | 树上的AI 专属 |
| `/homeclaw/` | homeclaw 专属 |
| `/ubuntu/` | ubuntu 专属 |
| `/flyingAI/` | flying'AI 专属 |
| `/BladeRunner/` | BladeRunner 专属 |
| `/公共/` | **共用文件**（各 agent 交付的共享产物多放一份） |

**上传规则：**
1. **所有上传**先落到自己专属目录：`/<你的名字>/文件名`
2. **共用/交付给用户的文件**：另外再传一份到 `/公共/文件名`
3. 专属目录放工作文件（中间产物、调试文件）；公共目录放最终交付物
4. 上传到子目录：`upload-link/?p=/目录` 拿链接后，POST 时 `parent_dir='/目录'` + `filename='纯文件名'`
